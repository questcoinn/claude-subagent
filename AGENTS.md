...
Claude Code 설정 산출물(서브에이전트 정의, 스킬)을 모아두는 콘텐츠 저장소다. 각 파일은 프로젝트의 `.claude/agents/`, `.claude/skills/`(또는 사용자 전역 `~/.claude/agents/`, `~/.claude/skills/`)로 복사해서 쓴다. 빌드·린트·테스트 도구는 없다 — 여기 있는 모든 파일은 실행되는 코드가 아니라 Claude Code 자신이 읽어들이는 마크다운 스펙이다.
...
<!-- codebase-context-engineer:begin (last_analyzed_commit: 0e611db3f1dba514a94ee953defca206b8879284, last_analyzed_date: 2026-09-11) -->
## AI를 위한 프로젝트 규칙

### 정본과 사본

- **정본은 저장소의 `agents/`, `skills/`다.** `.claude/agents/`에 같은 이름의 사본이 있을 수 있고(현재 `codebase-context-engineer.md`), 이건 사용자가 직접 복사해 넣은 것이다.
- 정본을 수정한 뒤 **`.claude/agents/`나 전역 `~/.claude/`로 자동 복사하지 않는다.** 사용자가 수동으로 동기화한다. 배포 자동화는 아직 없는 TODO 상태이므로(README 참고), 임의로 복사하면 사용자가 의도적으로 다르게 유지 중인 사본을 덮어쓸 수 있다.
- 반대로, 두 위치의 내용이 갈라져 보이면 조용히 맞추지 말고 사용자에게 알린다.

### 변경 검증 절차

자동 검증 수단이 없으므로, 문서를 고친 뒤 다음을 직접 확인하는 것이 유일한 "테스트"다.
...
3. 구조가 바뀌었으면 `docs/INDEX.md`의 파일 맵·섹션 위치 인덱스도 함께 갱신한다(줄 번호 포인터라 본문이 늘어나면 쉽게 어긋난다).

### 파일·경로 경계

- `docs/`는 `.gitignore`에서 통째로 제외되고 `!docs/CLAUDE.md`만 예외다. 따라서 **`docs/` 안의 인덱스 산출물은 커밋되지 않는다** — 팀과 공유해야 하는 내용을 `docs/`에만 적어두면 안 된다. (`!docs/CLAUDE.md` 예외는 이전 버전 지식 관리 에이전트가 쓰던 파일명의 잔재로 보이며, 현재 그 파일은 없다.)
- `.claude/knowledge-location.md`도 gitignore 대상이다. 이 저장소에는 시크릿·자격증명·`.env`가 없다(분석 시점 기준 전수 확인).
- `LICENSE`(MIT)는 손대지 않는다.

### 작성 언어

- 문서·스펙 본문은 한국어로 쓴다. 기술 식별자(frontmatter 키, 도구 이름, 경로)는 원문 그대로 둔다.
- 커밋 메시지는 영어 소문자 명령형으로 쓴다(예: `add license and readme file`). 커밋 2건 기준이라 확립된 규칙이라기보다 관측된 패턴이다.
